...
class ProdutoPedidosMiddleware {
  async create(req, res, next) {
    ...
    let schema = Yup.object({
      pedidoId: Yup.string().required(),
    });

    if (!(await schema.isValid({ pedidoId }))) {
      return res
        .status(500)
        .json({ erro: "pedidoId é obrigatório", campo: "pedidoId" });
    }
    schema = Yup.object({
...
module.exports = new ProdutoPedidosMiddleware();
