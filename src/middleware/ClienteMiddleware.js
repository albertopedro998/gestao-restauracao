...
class ClienteMiddleware {
  async create(req, res, next) {
    ...
    if (!(await schema.isValid({ nome }))) {
      return res.status(500).json({ erro: "Nome é obrigatório", campo: "nome" });
    }
    schema = Yup.object({
      telefone: Yup.string()
        .required()
    ...
    if (!(await schema.isValid({ telefone }))) {
      return res
        .status(500)
        .json({ erro: "telefone inválido, siga este padrão [000000000LA000]", campo: "telefone"  });
    }
    schema = Yup.object({
    ...
    if (!(await schema.isValid({ email }))) {
      return res.status(500).json({ erro: "Email é obrigatório", campo: "email"  });
    }
    schema = Yup.object({
      endereco: Yup.string(),
    });
    ...
    if (!(await schema.isValid({ endereco }))) {
      return res.status(500).json({ erro: "Endereço é obrigatório", campo: "endereco"  });
    }
