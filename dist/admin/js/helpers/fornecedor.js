...
async function fetchCreate() {
  const fields = ["nome", "nif", "telefone", "email", ,];
...
async function fetchUpdate() {
  const fields = ["nomeUpdate", "nifUpdate", "telefoneUpdate", "emailUpdate"];
...
$("#btnExcel").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Fornecedor.DATA) {
    let { id, nome, telefone, nif, email, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ id, nome, telefone, nif, email, createdAt });
  }
...
$("#btnPdf").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Fornecedor.DATA) {
    let { nome, telefone, nif, email, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ nome, telefone, nif, email, createdAt });
  }
  await fetch("http://localhost:3000/relatorios/pdf", {
    method: "POST",
    body: JSON.stringify({
      conteudo: {
        cols: ["Nome", "telefone", "Nif", "email", "Data do cadastro"],
        rows: datas,
