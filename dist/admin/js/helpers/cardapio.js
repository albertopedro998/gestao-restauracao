...
$("#btnExcel").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Cardapio.DATA) {
    let { id, nome, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ id, nome, createdAt });
  }
    ...
    body: JSON.stringify({
      conteudo: {
        cols: [
          ...
          { header: "Data da criação", key: "createdAt", width: 20 },
        ],
        rows: datas,
      },
    }),
...
$("#btnPdf").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Cardapio.DATA) {
    let { nome, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ nome, createdAt });
  }
  await fetch("http://localhost:3000/relatorios/pdf", {
    method: "POST",
    body: JSON.stringify({
      conteudo: {
        cols: ["nome", "Data do cadastro"],
        rows: datas,
