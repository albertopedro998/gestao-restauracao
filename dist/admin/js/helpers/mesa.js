    ...
    try {
      $("tbody").append(`
            <tr class="align-middle">
              ...
              <td>
                <span  class="badge ${
                  row.status.includes("ocupada") ? "text-white" : "text-primary"
                } ${row.status.includes("ocupada") && "text-bg-danger"} ">
                  ${row.status}
                </span>
              </td>
              <td>${new Date(
...
async function fetchUpdate() {
  const fields = ["numeroUpdate", "statusUpdate"];
...
let loading = false;
$("#numeroFiltro").keydown(async function (ev) {
  if (ev.keyCode == 13 && !loading) {
    loading = true;
    showRowsOnTable(await funcionario.index(`status=%${this.value}%`));
    loading = false;
  }
});
...
$("#btnExcel").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Mesa.DATA) {
    let { id, numero, status, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ id, numero, status, createdAt });
  }
...
$("#btnPdf").click(async function (ev) {
  ...
  let datas = [];
  for (const row of Mesa.DATA) {
    let { numero, status, createdAt } = row;
    createdAt = new Date(createdAt).toLocaleDateString();
    datas.push({ numero, status, createdAt });
  }
  await fetch("http://localhost:3000/relatorios/pdf", {
    method: "POST",
    body: JSON.stringify({
      conteudo: {
        cols: ["Numero", "Status", "Data do cadastro"],
        rows: datas,
